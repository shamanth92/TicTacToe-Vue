...
function boxClicked(index: number) {
  ...
  if (
    ...
    if (Object.values(boxState.value).every((val) => val === 'X' || val === 'O')) {
      drawn.value = true
      visible.value = true
    }
  }
...
<template>
  <div class="grid grid-cols-3">
    <div v-for="(item, n) in status" :key="n" class="p-3">
      <button
        :class="[
          'h-30 w-30  rounded-lg cursor-pointer text-4xl text-white font-serif',
  ...
  <div class="flex flex-row gap-3" v-if="startGameOption">
    <button class="bg-gray-500 h-10 w-30 rounded-lg text-white font-serif">
      {{ playerOne }}: X
    </button>
    <button class="bg-gray-500 h-10 w-30 rounded-lg text-white font-serif">
      {{ playerTwo }}: O
